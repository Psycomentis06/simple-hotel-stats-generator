...
if (isset($_POST['withincome'])) {
    ...
    if (empty($_POST['income'])) {
        array_push($errors, 'Income is missing');
    } else {
        $income = $_POST['income'];
    }
    ...
    if (count($errors) > 0) {
        ...
        $stat = new Stat($income, 0, 0, $host, $foodPercent, $drinkPercent);
        ...
        if ($isAjax == 'true') {
            ...
            foreach ($finalResult as $value) {
                ...
                //$htmlTabRes .= '<th scope="col"> <button class="btn btn-success" onclick="CreatePDF('. ($index+1) .');"><i class="fas fa-download"></i> PDF</button> </th>';
